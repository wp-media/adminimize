jQuery( document ).ready( function( $ ) {
	...
	for ( var i = 0; i < areas.length; i++ ) {
		...
		$( '#_mw_adminimize_options' ).find( "h3:contains('" + areas[ i ] + "')" ).parent().parent().find( 'table thead' ).first().find( 'th' ).each( function( x ) {
			...
			if ( x > 0 ) { // Skip the label column
				...
				$( this ).toggle( function() {
					$( this ).parent().parent().parent().find( "input[name*='" + userRole + "']" ).prop( 'checked', true );
				}, function() {
					$( this ).parent().parent().parent().find( "input[name*='" + userRole + "']" ).prop( 'checked', false );
				} );
