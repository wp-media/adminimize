...
?>
		...
		<div id="poststuff" class="ui-sortable meta-box-sortables">
			<div class="postbox">
				...
				<h3 class="hndle" id="backend_options"><?php _e('Backend Options', 'adminimize' ); ?></h3>
				<div class="inside">
					...
					<table summary="config" class="widefat">
						<tbody>
							...
							<tr valign="top" class="form-invalid">
								<td><?php _e( 'Use Global Settings', 'adminimize' ); ?></td>
								<td>
									<?php
									...
									if ( is_multisite() && is_plugin_active_for_network( MW_ADMIN_FILE ) ) {
										...
										$message = __( 'The plugin is active in multiste.', 'adminimize' );
									}
									$mw_adminimize_use_global = get_option( 'mw_adminimize_use_global' ); ?>
									<select name="_mw_adminimize_use_global"<?php echo $select_active; ?>>
										<option value="0"<?php if ( '0' === $mw_adminimize_use_global ) { echo ' selected="selected"'; } ?>><?php _e( 'False', 'adminimize' ); ?></option>
										<option value="1"<?php if ( '1' === $mw_adminimize_use_global ) { echo ' selected="selected"'; } ?>><?php _e('True', 'adminimize' ); ?></option>
									</select> <?php _e('Use the settings global in your Multisite network.', 'adminimize' ); echo ' ' . $message; ?>
								</td>
							</tr>
							-->
							<tr valign="top" class="form-invalid">
								<td><?php _e('Exclude Super Admin', 'adminimize' ); ?></td>
								<td>
									...
									<select name="_mw_adminimize_exclude_super_admin">
										<option value="0"<?php if ($_mw_adminimize_exclude_super_admin == '0') { echo ' selected="selected"'; } ?>><?php _e('Default', 'adminimize' ); ?></option>
										<option value="1"<?php if ($_mw_adminimize_exclude_super_admin == '1') { echo ' selected="selected"'; } ?>><?php _e('Activate', 'adminimize' ); ?></option>
									</select> <?php _e('Exclude the Super Admin on a WP Multisite Install from all limitations of this plugin.', 'adminimize' ); ?>
								</td>
							</tr>
							<?php } ?>
							<tr valign="top">
							...
							<tr valign="top">
								...
								<td>
									...
									</select> <?php _e('The Footer-area can hide, include all links and details.', 'adminimize' ); ?>
								</td>
							</tr>
							<?php 
							...
							<tr valign="top">
								<td><?php _e('Header', 'adminimize' ); ?></td>
								<td>
									<?php $_mw_adminimize_header = _mw_adminimize_get_option_value('_mw_adminimize_header'); ?>
									<select name="_mw_adminimize_header">
										<option value="0"<?php if ($_mw_adminimize_header == '0') { echo ' selected="selected"'; } ?>><?php _e('Default', 'adminimize' ); ?></option>
										<option value="1"<?php if ($_mw_adminimize_header == '1') { echo ' selected="selected"'; } ?>><?php _e('Hide', 'adminimize' ); ?></option>
									</select> <?php _e('The Header-area can hide, include all links and details.', 'adminimize' ); ?>
								</td>
							</tr>
							<?php } // end if < wp 3-6 ?>
							<tr valign="top">
							...
							<tr valign="top">
								...
								<td>
									...
									</select> <?php _e('All Thickbox-function use the full area of the browser. Thickbox is for example in upload media-files.', 'adminimize' ); ?>
								</td>
							</tr>
							<tr valign="top">
							...
							<?php } // end if < wp 3-6 ?>
							<tr valign="top">
								<td><?php _e('Category Height', 'adminimize' ); ?></td>
								<td>
									<?php $_mw_adminimize_cat_full = _mw_adminimize_get_option_value('_mw_adminimize_cat_full'); ?>
									<select name="_mw_adminimize_cat_full">
										<option value="0"<?php if ($_mw_adminimize_cat_full == '0') { echo ' selected="selected"'; } ?>><?php _e('Default', 'adminimize' ); ?></option>
										<option value="1"<?php if ($_mw_adminimize_cat_full == '1') { echo ' selected="selected"'; } ?>><?php _e('Activate', 'adminimize' ); ?></option>
									</select> <?php _e('View the Meta Box with Categories in the full height, no scrollbar or whitespace.', 'adminimize' ); ?>
								</td>
							</tr>
							<tr valign="top">
